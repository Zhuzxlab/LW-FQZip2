...
{
	char* meta_buffer = m_malloc(read_len);
	char* meta_temp = m_malloc(read_len);
	char* meta_final = m_malloc(read_len);
	fgets(meta_temp,read_len,fastq);
	fgets(meta_buffer,read_len,fastq);
	fgets(meta_buffer,read_len,fastq);
	if(meta_buffer[0]=='+'&&meta_buffer[1]=='\n'){//如果第三行只有"+"则把"+"放到文件开头
		meta_final[0] = '+';
		strcat(meta_final,meta_temp);
		fputs(meta_final,meta);
		//fputc('\n',meta);
	}else{
...
void  FQZip_compression(char *fastq_name, unsigned long long totalread,int BlockNum, char *ref_name, int assemble_flag,int highestCom_flag)
{
	...
	if (t == 1)   //not LWFQZip2 dir
	{
		...
		sprintf(order,"%s %s %s ","tar -c -P -f ",lwfqzip_name,tar1_name);
		if (-1 == (system(order)))printf("Please make sure you have installed the tar tool correctly");
		//printf(order,"%s %s %s ","tar -c -f ",lwfqzip_name,tar1_name);
		//if (-1 == (system(order)))printf("Please make sure you have installed the tar tool correctly");
		chdir(local_dir);
	}
	if (t==0)       //LWFQZip2 dir
...
void LWFQZip_compression(char *input_name,char *ref_name,int assemble_flag, unsigned long long totalread,int BlockNum,int highestCom_flag)
{
	...
	//printf("input_name=%s\ntotalread=%lld\nBlockNum=%d\nref_name=%s\nassemble_flag=%d\nhighestCom_flag=%d\n",input_name, totalread,BlockNum,ref_name,assemble_flag,highestCom_flag);
	//printf(" aaaa input_name is %s\n",input_name);
